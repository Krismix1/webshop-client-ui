...
describe('App: Products Filter', () => {
 beforeEach(() => {
   ...
   describe('FilterProducts', () => {
    const pipe = new FilterProducts()
    it('1. No search string returns nothing', () => {
        const results = pipe.transform(this.products, '')
        expect(results.length).toBe(0)
    })

    it('2. Empty array returns empty array', () => {
        const results = pipe.transform([], 'Hi')
        expect(results.length).toBe(0)
    })

    it('3. Valid input and args', () => {
        const results = pipe.transform(this.products, 'desc a')
